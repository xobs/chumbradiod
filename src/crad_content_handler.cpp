...
chumby::HTTPResponse * ChumbRadioContentHandler::handleRequest(const chumby::HTTPRequest & request)
{
    ...
    else if(baseURI == configURI)
    {
        ...
        /*! process parameters */
        {
            ...
            for(v=0;v<paramList.size();v++)
            {
                ...
                {
                    if(!strcasecmp(cur_value.c_str(), "usa"))
                        country = COUNTRY_USA;
                    if(!strcasecmp(cur_value.c_str(), "japan"))
    ...
    else if( (baseURI == serviceStartURI) || (baseURI == serviceStopURI) || (baseURI == serviceStatusURI) )
    {
        ...
        {
            FILE *inp_file = popen("service_control chumbradioplay start", "r");
            pclose(inp_file);
        }
        ...
        {
            FILE *inp_file = popen("service_control chumbradioplay status", "r");
